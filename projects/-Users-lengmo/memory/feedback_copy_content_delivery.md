...
**How to apply:**
- **短内容**（几句话的文案/结论/短反馈，无论是否要存文档）：对话里直接给**纯文本**——不用表格/引用块/`#`/`**`，写成连续段落或单行。我自己在终端里的分析/结论可以照常用 markdown（表格便于看，见 [[feedback_writing_taste_umbrella]]）。soft-wrap 换行的残留用户自己接受，别主动推荐写文件。
