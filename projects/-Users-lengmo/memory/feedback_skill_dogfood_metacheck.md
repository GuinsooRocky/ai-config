...
**How to apply:** 写完/大改 skill 的最后一步固定是跑 meta-check-skill。多轮编辑尤其危险——改一处容易忘了同步其他处（drift）。审计重点看 Drift 维度（引用路径是否存在、章节间指令是否一致）。相关：[[feedback_skill_design_pattern]]、[[feedback_skill_personal_only]]。
