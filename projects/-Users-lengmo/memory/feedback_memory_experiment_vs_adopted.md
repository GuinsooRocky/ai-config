...
**Why:** 2026-05-14 在 onlychat-world-book worktree 起 dev，pnpm install 报 node_modules missing。我看见 [[project_onlychat]] 里关联的 bun 升级实施记录（World-Path 的归档目录叫 `onlychat-World-Path/历史记录/`，不叫 `archive/`；那份 bun 记录如今已不在库里），就把 bun 当作"主仓已升级"的事实，给用户摆了一个"pnpm vs bun rebase vs symlink"三选一的问题。实际上 worktree 只有 `pnpm-lock.yaml`，`package.json` engines/packageManager 也只写 pnpm — lockfile 已经把答案写死了。用户回："那个只是我本地的一个测试，还没有推。"
...
- 关联记忆：[[project_onlychat]]；那份 bun 升级记录是实验记录，不是已落地状态——2026-07-06 复核四个 worktree 仍全是 pnpm。
