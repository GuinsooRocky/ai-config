...
metadata: 
  ...
  modified: 2026-08-13T04:45:05.111Z
---
...
- **写结论别写思辨过程**（2026-08-13 点名，SQL 注释 `-- 拆系统画；不是因为尺子不同，是别让机型分布冒充实验效果` → 只留 `-- 按系统拆`）：注释里出现「不是因为 A，是 B」「权衡后选了 X」「本来想 Y」这种把推导过程摊开的写法一律砍，只留最终结论那一句。**非直觉 why 仍要留，但写成陈述句不写成论证**（`// -1 = 没走到终点，不能当 0 混进分位数` OK；「之所以用 -1 而不用 null 是因为…」不 OK）。同源 [[feedback_deliverable_is_final_state_not_process]]
