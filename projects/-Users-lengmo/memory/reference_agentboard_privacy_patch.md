...
metadata: 
  ...
  modified: 2026-09-01T08:50:08.475Z
---
...
- **⭐v3 cwd 升级**：peekaboo 原本 `project_path` 认「首条 cwd、之后不覆盖」（parseTranscripts line ~4279），从 home 开局的会话即便中途 cd 进 cmm 也被钉死成 personal 被 drop。v3 改成「一旦出现 cmm cwd 就升级过去、不降级」（自包含内联 `__abPrivacyIsWork`，不依赖注入块作用域）→ home 启→cd 进 cmm 的会话自动按 cmm 真实路径上报，**不用逐个加名单**
