...
**How to apply:** 调研类 workflow（ultracode/deep-research/agent 选型）收尾必须包含"对已有 skill/agent 的回写清单"；推荐新建任何东西之前先问"能不能并进现有资产"；落地后跑对应审计（meta-check-skill ≥85）。相关：[[feedback_inline_over_background_workflow]]、[[feedback_skill_design_pattern]]（meta-check ≥85 那条现居此文件）。
