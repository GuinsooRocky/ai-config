...
**How to apply**：cherry-pick 密集的仓（onlychat 三条 release 线每周互相 cp）里，任何「这 commit 在不在 X 分支」的判断一律按内容验。人给的口头事实（「我已经发上线了」「已经 cp 过了」）跟工具结论打架时，先怀疑工具用法——尤其在拿它当理由去阻拦 owner 的时候。同源原则见 [[feedback_truncated_output_is_not_ground_truth]]（按内容特征校验、别凭状态码下结论）、[[feedback_dont_declare_infeasible]]（别急着替 owner 判死刑）。
