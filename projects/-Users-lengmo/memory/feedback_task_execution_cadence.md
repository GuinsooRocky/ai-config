...
**大改动档（动 type/接口、公共组件、一大块逻辑、提 PR 前）→ 写完当场跑增量 `tsc`**（onlychat/世界卡系列）。
- Why：dev（SWC/Turbopack）只转译不查类型，pre-commit（`pnpx lint-staged`）只跑 eslint+prettier 也不查类型。**用户不开 VS Code/Cursor，没有编辑器红线**——写完手动跑那一次是最早的类型防线。
- **2026-07-14 现实更新（原「别加 typecheck script」那条作废）**：团队自己把闸加上了——`17aef9105f`（Wei，perf: add TypeScript 7 typecheck gate）给 `package.json` 加了 `typecheck`（typescript7 + `--tsBuildInfoFile .next/cache/ts7.tsbuildinfo`，天然增量），`.husky/pre-push` 加了 blocking 的 `tsc type check` 与 `verify:gates`。所以现在直接 `pnpm typecheck`，别再自己拼 `tsc --noEmit --incremental`，更别以为「加进仓会强加给团队」——闸已经是团队的了。**节奏本身不变**：pre-push 才拦等于失败发生在最后一步，大改动仍要写完当场自查，别留到 push 才发现。
- 曾评估 `tsc --watch` 常驻（吃内存，与防 OOM 冲突），放弃。内存高占用主犯是常驻 dev server（见 [[project_worldbook_dev_oom]]），不是一次性 tsc。
