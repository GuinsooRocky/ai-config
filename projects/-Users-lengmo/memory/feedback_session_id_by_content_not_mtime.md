...
**How to apply:** 按**内容指纹**认——grep 各候选 jsonl 找只有当前对话才有的字符串（本轮如 `6d487fa99`/`pin-gold`/`重锚`/`世界卡`/`agg-tuning`），命中的那个才是。mtime 顶多在"多个都内容命中"时做次级排序。相关：[[project_codegen_workflow]]。任何扫 jsonl 的活都同理：按内容归属，别假设一个目录 = 一个会话。
