...
**How to apply:** Figma 来源是 mobile 节点、或用户措辞含"mobile 端/手机"→ 默认 **mobile-only 门控**，PC 分支保持原实现，不要合并成一条。用户说"只需要改下 X 的排版"≠"PC 也一起改"。拿不准就按 mobile-only 做（PC 零回归是安全侧），需要统一时由用户显式确认。跟 [[project_onlychat_env_pitfalls]] 互补：那条讲"改共用组件两条轴都要顾"，这条讲"改动要落在它该落的那一格、别外溢"。
