...
- 「臂」：2026-08 两天被纠十几次（原话「我这两天说了 10 遍」）。已把 onlychat 全仓（src+docs，32 文件 202 处）机械换成「组」，存量为 0——再出现就是新引入回归，没有「存量照抄」借口。凡碰 worldcard 转场 / search 转场这类 A/B 埋点任务（那些文件的旧上下文会把这个字带出来），发消息/写注释/写用例名之前先自己搜一遍「臂」。
