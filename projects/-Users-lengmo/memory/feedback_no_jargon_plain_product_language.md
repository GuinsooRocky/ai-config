...
description: "对产品/非技术读者说话：大白话 + 禁用词表（臂/客户端/北极星…）+ 群里改动说明按 动作→应该看到→实际看到 写、原因一句生活比喻"
metadata:
  node_type: memory
  type: feedback
...
**群里改动说明的固定句式（2026-09-07 onlychat 老板三轮返工，合并自 feedback_group_message_for_product_readers）：** 老板原话「上面的描述就是问题了 → 页面应该显示搜索页，但是显示了首页」「离开什么到了哪里？」「这个过程好像说多了，产品可能看不懂」。我写的「页面记录带着首页的身份」「只影响 Mobile 搜索浮层」全被判读不懂。
- 每个问题一条：做了什么动作 → 应该看到什么 → 实际看到什么；延伸现象说清「到了哪里」，不留「才真正离开」这种悬空词
- 原因只一句，用生活比喻（「浏览器以为你还在首页」），不出现 history / pushState / 路由 / 实验组 / 浮层
