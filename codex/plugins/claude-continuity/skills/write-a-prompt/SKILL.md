...
### Step 1 — 补齐意图（先重述真问题，再反问一个）

**先把「用户嘴上要的 prompt」和「他真正想让执行方达成的结果」分开重述一遍**。用户常说「写个 prompt 让 X 干 Y」，而 Y 往往只是他顺手想到的手段，真实目标在 Y 后面。真问题没定住，下面 4 要素收得再齐也是齐在错的问题上。

然后确认 4 要素齐不齐：**目标**（干成什么）/ **谁跑它**（主 session？子 agent？有无浏览器工具）/ **成功长啥样**（输出什么、写哪、怎么算完）/ **绝不做什么**（红线）。缺的**必须**先反问，别硬编——但**一次只问杠杆最大的那一个**，等回答再问下一个，别一口气甩四问（同 grill-me 规则 1）。
...
按 6 条质量线成文。**无论长短，prompt 一律直接贴在对话里**（代码块包裹保持纯文本）——用户要的是当场复制粘去别的 session，落 scratchpad/文档都是过度动作，且 scratchpad 路径用户点不开（2026-07-26、07-30 两次纠正）。只有用户明说"存下来/落到 xx"才写文件。交付后一句话说清：这份 prompt 谁来跑、跑前要先核实哪几点。
