---
name: firecrawl
description: Firecrawl CLI：整站 URL 清单（`map`）、整站抓取（`crawl`）、关键词搜索带正文（`search`）、需要点击 / 登录才拿得到内容的页面（`interact`）。触发词："爬整站"、"整站抓"、"列出这站所有页面"、"搜一下带正文"、"抓这个文档站"、"要登录才能看的页面"。不用于：英文站单页（WebFetch 够用）、微信 / 小红书 / X / B 站 / YouTube（归 analyzer，云端抓这些站会吃风控页）、本地文件 / git / 部署 / 改代码。
allowed-tools:
  - Bash(firecrawl *)
  - Bash(npx firecrawl *)
---
...
<!-- 本机只保留这一个 firecrawl skill：官方 init 装的另外 31 个（search/scrape/crawl/map/interact/
     build-*/workflows 等）已移到 ~/.Trash/firecrawl-skills-20260730/，原因是 description 常驻
     上下文且与本机 analyzer / deep-research 抢活。具体子命令用法看本文件 Workflow 段或 `firecrawl <cmd> --help`。
     中文平台（微信/小红书/X/B站/播客）一律走 sf-reader-all，不用 firecrawl。 -->

- **抓中文平台链接** -> 不用本 skill，走 `analyzer` skill / sf-reader-all（云端 IP 抓 mp.weixin 会吃风控页）
