---
name: xhs-writer
description: 数字生命卡兹克（Khazix）的写作 skill，覆盖四条产出线，公众号长文写作、小红书 9 宫格图文帖（按 idea/文档生成 HTML 预览 + 迭代修改 + 一键导出图）、去 AI 味（把现成文字改得不像 AI 写的）、选题（从近期会话记录/素材挖选题卡 + 手贴后台数据复盘归因）、配图（小黑式手绘概念图：挑认知锚点出 shot list + 可粘 prompt，用户拿去自有网站出图，本机不出图）。公众号长文可按需使用丹·哈蒙故事圆环做叙事增强。触发词："配图"、"出配图 prompt"、"画个小黑"、"这篇哪里值得配图"、"写文章"、"写稿子"、"帮我写"、"续写"、"扩写"、"公众号长文"、"出稿"、"按我的风格写"、"叙事增强"、"八步叙事"、"故事圆环"、"做成小红书"、"出小红书"、"根据 idea/文档出小红书"、"小红书 9 图"、"改小红书帖"、"去 AI 味"、"去 xhs"、"改得不像 AI 写的"、"这段太 AI 了"、"挖选题"、"小红书选题"、"今天有啥可发的"、"复盘笔记数据"，丢 PDF/brief/新闻链接/语音转文字说"写成文章"也触发；小红书线详见 `references/xiaohongshu.md`，选题线详见 `references/topic-mining.md`，配图线详见 `references/illustration.md`，不用于推特/朋友圈短内容或纯标题摘要（用 `wechat-title` skill）。
---
...
这个 skill 有四条产出线，动手前先认准用户要哪条（触发词见 frontmatter）：

| 用户的话 | 走哪条 | 怎么做 |
|---|---|---|
| 写公众号文章 / 长文 / 续写扩写 / 叙事增强 | **公众号长文** | 走下面「核心价值观 → 第一步~第四步」完整流程；叙事类素材按需使用故事圆环 |
| 「根据这个 idea / 文档出小红书」「做成小红书帖」「改小红书」 | **小红书 9 图帖** | 读 `references/xiaohongshu.md`，按它走 |
| 给一段现成文字说「去 AI 味 / 去 khazix」 | **去 AI 味** | 不重写内容，只按「第三步·绝对禁区」+「第四步·L1 硬性规则检查」逐条扫改；改完再过 `references/lengmo-taste-rulings.md` 的五问自查 |
| 「挖选题」「今天有啥可发的」「复盘笔记数据」 | **选题** | 读 `references/topic-mining.md`，按它走（产 2-4 张选题卡 / 复盘归因，不直接动笔写正文） |

触发词：写文章、写稿子、帮我写、续写、扩写、公众号长文、出稿、按我的风格写、叙事增强、八步叙事、故事圆环、做成小红书、出小红书、根据 idea/文档出小红书、小红书 9 图、改小红书帖、去 AI 味、去 khazix、改得不像 AI 写的、这段太 AI 了、挖选题、小红书选题、今天有啥可发的、复盘笔记数据。

两条铁律：
- **小红书帖生成时，边写边套去 AI 味规则**（第三步「绝对禁区」+ 第四步 L1），不要先写脏再洗。
- 拿不准用户要哪条线，直接问一句，别猜。

另：任何产出线交稿前，过一遍 `references/lengmo-taste-rulings.md`（号主真实判例集：注水/搭台子/双写/保原话/黑话/精准×共情，末尾有五问自查）。禁区管词汇层，判例管句子结构层，判例权重更高——规则可辩，判例不可辩。

下面的「核心价值观」「第一步~第四步」是公众号长文的完整人格写作流程；小红书线只复用其中的去 AI 味规则，其余一切按 `references/xiaohongshu.md`。
...
用户会给你任何形式的输入，产品brief、新闻链接、PDF、语音转文字、散乱想法、一段开头加几个要点。
...
**丹·哈蒙故事圆环（可选叙事增强）**：调查实验型、产品体验型、带真实行动的工具分享型，优先用故事圆环梳理素材。现象解读型只在确实存在“追问 → 研究 → 改变理解”时使用。纯方法清单、速查教程和信息汇总默认不套。

动笔前可以在草稿中写一行八步骨架，但不要把英文步骤当成正文小标题：

`You 现状 → Need 缺口 → Go 进入未知 → Search 尝试与学习 → Find 得到结果 → Take 付出代价 → Return 回到原问题 → Change 理解或行动已经改变`

写非虚构文章时，把八步还原成真实素材：从一个具体场景开始，说清什么问题或好奇心打破了平衡，再跟着真实尝试、证据和失败往前走。「Find」不一定是成功，也可以是一个意外结论；「Take」必须是真实的时间、失败、限制或认知代价，没有就不要编；「Return」要回到开头的场景、问题或意象；「Change」要让读者看到，同一件事现在已经有了不同的含义。

八步可以合并、弱化或省略，不为补齐结构虚构经历、对话、情绪或代价。如果素材撑不起完整圆环，至少守住三个点，「Need」让文章有一个真问题，「Return」让结尾回到这个问题，「Change」让读者的理解不再停留在原地。这是结构工具，不是写作痕迹。
...
**L3-7 叙事闭环检查**
- 文章是否有一个真实、具体的 `Need`，而不是为了套故事硬造冲突？
- 如果使用叙事增强，`Go / Search / Find / Take` 是否都有真实行动、证据或限制支撑？
- 结尾是否 `Return` 到开头的场景、问题或意象，并让读者看到了一个具体的 `Change`？
- 如果本文不适合故事圆环，是否明确判定为不适用，而不是机械补齐八步？

**通过标准**：L3-1和L3-2必须全部通过，L3-3至L3-7中至少通过相关项（按文章类型判断适用性，不适用的项直接跳过并注明）。
...
选题线的执行流程（挖选题选题卡格式、数据复盘漏斗与权重文件），参考 `references/topic-mining.md`。
